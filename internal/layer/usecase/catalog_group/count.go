...
func (uc *usecaseImpl) Count(ctx context.Context, req *request.Filter) (int64, error) {
	...
	query := req.ToQuery()
	total, err := uc.catalogGroupRepo.Count(ctx, query)
	if err != nil {
		log.Error().Err(err).Interface("query", query).Msg("unable to count group")
		return 0, errors.Wrap(err, "unable to count group")
	}
